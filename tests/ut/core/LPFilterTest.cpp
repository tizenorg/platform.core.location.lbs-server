...
TEST_F(LPFilterTest, PureWave)
{
	...
	for (unsigned fs = MIN_SAMPLING_FREQUENCY_mHz;  fs <= MAX_SAMPLING_FREQUENCY_mHz;  fs *= 2) {
		result = _lpFilter.reset(fs*real(1.0 / FREQUENCY_UNIT));
		...
		if (result == Result::OK) {
			...
			for (unsigned fp = MIN_PROBE_FREQUENCY_mHz;  2 * fp <= fs;  fp *= 2) {
				...
				sum_u2 = 0;
				sum_v2 = 0;
				omega = fp * real(2 * M_PI) / fs;
				t0 = rand() * real(1.0/(RAND_MAX + 1));
				...
				real dB = 3 * log2(G);
				if (fp <= LPF_CUTOFF_FREQUENCY_HZ / 2 * FREQUENCY_UNIT)
					EXPECT_LE(square(dB), 1);
				else if (fp >= LPF_CUTOFF_FREQUENCY_HZ * 2 * FREQUENCY_UNIT)
					EXPECT_LE(dB, -12);
		...
		if (result == Result::OK) {
			...
			for (unsigned fp = MIN_PROBE_FREQUENCY_mHz;  2 * fp <= fs;  fp *= 2) {
				...
				for (unsigned t = 0;  t < testInterval;  ++t) {
					...
					r = LPF_GRAVITY + sin((t0 + t) * omega);
					// collect i/o for comparison
					real u = square(r) - LPF_GRAVITY * LPF_GRAVITY;
					sum_u2 += square(u);
					sum_v2 += square(_lpFilter.process(u));
				}
				real G = sum_v2/sum_u2;
