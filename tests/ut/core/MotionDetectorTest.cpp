...
static constexpr real TEST_START_TIME   = static_cast<real>(3.14);  // [s] anything greater than zero
static constexpr real TEST_INTERVAL     = static_cast<real>(100.0); // [s]
static constexpr real SAMPLING_INTERVAL = static_cast<real>(1.0 / MotionDetector::SAMPLING_FREQUENCY);
static constexpr real WALKING_FREQUENCY = static_cast<real>(2.0);   // [Hz]
static constexpr real WALKING_AMPLITUDE = static_cast<real>(2.0);   // [m/s^2]
...
TEST_F(MotionDetectorTest, Idle)
{
	...
	for (real dt = 0;  dt < TEST_INTERVAL;  dt += SAMPLING_INTERVAL)
...
TEST_F(MotionDetectorTest, GravityCalibration)
{
	...
	for (real g = MIN_G;  g <= MAX_G;  g += static_cast<real>(1.0 / 16)) {
		...
		for (real dt = 0; dt < 4 * CALIBRATION_INTERVAL; dt += SAMPLING_INTERVAL) {
...
TEST_F(MotionDetectorTest, Itinerant)
{
	...
	for (unsigned n = 0;  n < CYCLES;  ++n) {
		// simulate walk
		for (real dt = 0; dt < TEST_INTERVAL; dt += SAMPLING_INTERVAL) {
			av[SPATIAL_Z] = WALKING_AMPLITUDE * sin(dt * WALKING_FREQUENCY) + earth::GRAVITY;
		...
		__receivedImmobilityTriggers = 0;
		// simulate rest
		for (real dt = 0; dt < IMMOBILITY_INTERVAL + 4 * INTEGRATION_INTERVAL; dt += SAMPLING_INTERVAL) {
			av[SPATIAL_Z] = NOISE_LEVEL * sin(dt * WALKING_FREQUENCY) + earth::GRAVITY;
			onAccelerationData(t, av);
			t += SAMPLING_INTERVAL;
