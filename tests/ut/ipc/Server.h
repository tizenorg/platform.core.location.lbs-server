...
struct GApplication {
	...
	void stop() { ::g_main_loop_quit(loop); }

	~GApplication() { ::g_main_loop_unref(loop); }
};
...
public:
	...
	virtual bool
	subscribe(const Server::ClientId&, const unsigned, std::unique_ptr<Server::ClientProxy>&&)
	{
		return true;
	}
};
...
struct ServerCommunicationTest : public ServerMock, public ::testing::Test {
	 ...
	 */
	template <typename T> void asyncTest(T&& function)
	{
