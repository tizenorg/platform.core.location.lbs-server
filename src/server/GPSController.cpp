...
void GPSController::start()
{
	...
	int ret = ::lbs_client_create(LBS_CLIENT_METHOD_GPS, &__lbsHandle);
	if (ret != 0) {
	...
	auto signalCb = [](const gchar* signal, GVariant* parameters, gpointer user_data) {
		...
		if (!g_strcmp0(signal, "PositionChanged")) {
			controller->__onPositionChanged(parameters);
		} else if (!g_strcmp0(signal, "StatusChanged")) {
			controller->__onStatusChanged(parameters);
		} else {
			LOGD("Invaild signal " + std::string(signal));
		}
	};

	ret = ::lbs_client_start(__lbsHandle, INITIAL_INTERVAL, LBS_CLIENT_LOCATION_CB, signalCb, this);
	if (ret != 0) {
...
void GPSController::updateInterval(const std::chrono::milliseconds& interval)
{
	...
	int ret = ::lbs_client_set_position_update_interval(__lbsHandle, interval.count());
	if (ret != 0) {
...
{
	if (__lbsHandle != nullptr) {
		::lbs_client_destroy(__lbsHandle);
	}
}

void GPSController::__onPositionChanged(GVariant* parameters)
{
	...
	__updatePosition(position);
}

void GPSController::__onStatusChanged(GVariant* parameters)
{
...
void GPSController::__updatePosition(const core::Position& position)
{
	__receiver.updatePosition(position);
}
}
