...
std::string Server::echo(const std::string& message)
{
    return message;
}
...
bool Server::unregisterClient(const ipc::Server::ClientId& clientId)
{
    ...
    if (ret != clients.end())
    {
        ...
        LOGI("New general settings: " + levelToString(maxAccuracyLevel)
                                      + ", desired interval: "
...
bool Server::setDesiredInterval(const Server::ClientId& clientId, const unsigned interval)
{
    ...
    if (ret != clients.end())
    {
        ...
        LOGI("The desired interval of (" + clientId + ") was set to "
