...
void Server::onNameLost() { LOGE("DBus service name lost."); }
                          ...
                          GDBusMethodInvocation* invocation)
{
	...
	if (method == "echo") {
		...
		::g_dbus_method_invocation_return_value(invocation, g_variant_new("(s)", response.c_str()));
		...
		try {
			...
			::g_dbus_method_invocation_return_value_with_unix_fd_list(
			    invocation, g_variant_new("(h)"), fdlist);

			g_object_unref(fdlist);
		} catch (const PipeException& e) {
			::g_dbus_method_invocation_return_dbus_error(
			    invocation, "org.tizen.lbs.Providers.FusedLocation.SubscribtionError", e.what());
		}
	} else {
...
void Server::onAnyNameVanished(const std::string& name) { handler.unregisterClient(name); }
...
void Server::registerClient(const Server::ClientId& clientId)
{
	...
	auto freePair = [](gpointer ptr) { delete static_cast<std::pair<Server*, guint>*>(ptr); };
...
int Server::ClientProxy::getInput() const { return fd[0]; }
