...
void Client::setPositionCallback(Client::PositionCb positionCb) { positionCallback = positionCb; }

void Client::unsubscribe() { LOGW("Unsubscribe is not implemented yet!"); }

bool Client::isConnected() const { return connection != nullptr; }
...
void Client::sourceFd(int fd)
{
	...
	source = ::g_pollable_input_stream_create_source(
	    reinterpret_cast<GPollableInputStream*>(stream), nullptr);
...
bool Client::readAllData(void* buffer, size_t size)
{
	...
	while (read < size) {
		gssize ret =
		    ::g_pollable_stream_read(stream, ptr + read, size - read, false, nullptr, error.pass());
