...
static void start_tracking(lbs_server_s *lbs_server, lbs_server_method_e method)
{
	...
	switch (method) {
		...
		case LBS_SERVER_METHOD_MOCK:
			...
			LOG_GPS(DBG_LOW, "start_tracking MOCK");
			...
			if (ret) {
				...
				LOG_GPS(DBG_ERR, "Failed to mock_start_tracking");
			}
			break;
	...
	gdouble speed, gdouble direction, gdouble accuracy, gpointer userdata)
{
	...
	lbs_server->mock_position.latitude = latitude;
	lbs_server->mock_position.longitude = longitude;
	lbs_server->mock_position.altitude = altitude;
	lbs_server->mock_position.hor_accuracy = accuracy;
	lbs_server->mock_position.speed = speed;
	lbs_server->mock_position.direction = direction;

	LOG_SEC("lat = %lf, lng = %lf", lbs_server->mock_position.latitude, lbs_server->mock_position.longitude);
}
...
static gboolean __mock_position_update_cb(gpointer userdata)
{
	...
	time_t timestamp;
	time(&timestamp);
	LOG_MOCK(DBG_LOW, "time:%d", timestamp);
	...
	lbs_server_emit_position_changed(lbs_server->lbs_dbus_server, LBS_SERVER_METHOD_MOCK,
									lbs_server->mock_position.fields, lbs_server->mock_position.timestamp, lbs_server->mock_position.latitude,
									lbs_server->mock_position.longitude, lbs_server->mock_position.altitude, lbs_server->mock_position.speed,
									lbs_server->mock_position.direction, 0.0, lbs_server->mock_accuracy);
...
static gboolean mock_start_tracking(lbs_server_s *lbs_server)
{
	...
	if (lbs_server->mock_position.direction >= 0) {
...
static void mock_set_status(lbs_server_s *lbs_server, LbsStatus status)
{
	...
	LOG_NPS(DBG_LOW, "staus = [%d]", status);
	if (lbs_server->status == status) {
...
{
	LOG_MOCK(DBG_LOW, "remove_all_clients GPS");
	if (lbs_server->mock_client_count <= 0) {
		lbs_server->mock_client_count = 0;
		return FALSE;
	}
